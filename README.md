...
2. **Set `SIGNUP_INVITE_CODE`.** Registration is gated on this: pick any
   phrase, add it as an environment variable, and anyone creating an account
   has to enter it. If it isn't set, registration is off entirely — open by
   default would mean anyone who found the URL could sign up.

3. **Create your account.** Visit the deployed site, choose *Create an
   account*, and enter your email, a password, and the invite code.
...
Each account gets its own private ledger. Every transaction and balance row
carries a `user_id`, and every query is scoped to the signed-in user, so two
people can even have accounts with the same name ("HDFC ...9939") without
seeing each other's numbers. Deleting a row from `users` deletes that person's
data with it.

To add someone, give them the invite code. To stop new sign-ups, remove
`SIGNUP_INVITE_CODE` and redeploy; rotating it invalidates any code you've
already handed out.

**There is no password reset.** If someone forgets their password, you have to
fix it by hand — generate a hash with `hashPassword()` from `lib/auth.ts` and
`UPDATE users SET password_hash = ...`. Self-service reset needs an email
provider and a token table, which isn't built. Worth knowing before inviting
anyone who isn't within arm's reach.

Two other limits to be aware of: changing a password does not sign out other
devices (sessions are stateless signed tokens and stay valid until they
expire), and there's no rate limiting on sign-in attempts.
